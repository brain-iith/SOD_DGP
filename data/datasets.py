...
class Dataset(object):
    ...
    """
    Credits : https://github.com/ICL-SML/Doubly-Stochastic-DGP/blob/master/demos/datasets.py
    """
    ...
    def normalize(self, split_data, X_or_Y):
        ...
        s = np.std(split_data[X_or_Y], 0)[None, :] + 1e-6 #s = np.std(split_data[X_or_Y + 's'], 0)[None, :] + 1e-6
...
class StepFunction(Dataset):
    def __init__(self):
        self.name, self.N, self.D = 'step_function', 50, 1
...
class Airfoil(Dataset):
    def __init__(self):
        self.name, self.N, self.D = 'airfoil', 1503, 5
        self.type = 'regression'
        self.norm = True

    def download_data(self):
        url = '{}{}'.format(uci_base, '00291/airfoil_self_noise.dat')
        data = pandas.read_csv(url, header=None, sep=' ').values
        with open(self.csv_file_path(self.name), 'w') as f:
            csv.writer(f).writerows(data)
            
class Toxic2(Dataset):
    def __init__(self):
        self.name, self.N, self.D = 'toxic2', 546, 8
        self.type = 'regression'
        self.norm = True

    def download_data(self):
        url = '{}{}'.format(uci_base, '00505/qsar_aquatic_toxicity.csv')
        data = pandas.read_csv(url, header=None,sep = ';').values
        with open(self.csv_file_path(self.name), 'w') as f:
            csv.writer(f).writerows(data)
            
class Toxic1(Dataset):
    def __init__(self):
        self.name, self.N, self.D = 'toxic1', 908, 6
        self.type = 'regression'
        self.norm = True

    def download_data(self):
        url = '{}{}'.format(uci_base, '00504/qsar_fish_toxicity.csv')
        data = pandas.read_csv(url, header=None, sep = ';').values
        with open(self.csv_file_path(self.name), 'w') as f:
            csv.writer(f).writerows(data)
...
class Year(Dataset):
    def __init__(self):
        self.name, self.N, self.D = 'year', 515344, 89
        self.type = 'regression'
        self.norm = True
    ...
    def download_data(self):
        ...
        data = pandas.read_csv('/tmp/YearPredictionMSD.txt', delimiter = ',').values
        data = np.concatenate([data[:, 1:], data[:, 0, None]], 1)
...
class Datasets(object):
    def __init__(self, data_path='/data/'):
        ...
        datasets.append(StepFunction())
        datasets.append(Airfoil())
        datasets.append(Toxic1())
        datasets.append(Toxic2())
